...
- `src/serve_web.py` + `src/serve_control.py` + `web/*` -- local servers: `serve_web.py` provides chat UI (chat, streaming, models, sessions, flags) and `serve_control.py` provides the full control API (tasks, training, memory, maintenance, torickle, swarm, research).
...
- `src/federated/nat_traversal.py` -- real NAT traversal for `SwarmNode`/`InferenceNode`: STUN (RFC 5389) discovers the machine's actual public IP instead of a local-interface guess, and UPnP IGD asks the router to auto-forward the listening port. Opt-in (`start(attempt_nat_traversal=True)` / CLI `--nat-traversal` / the web UI's "Join peer network" toggle) since it makes outbound network calls and a router change; best-effort and never fatal. Doesn't solve peer *discovery* (finding a first address with no prior contact) -- see `docs/BOOTSTRAP_NODE.md` for why that's a hosting decision, not a code one.
...
- `src/federated/codistill.py` -- cross-architecture co-distillation: peers with *different* model sizes/shapes (nano/laptop/desktop) teach each other by exchanging text answers to a shared probe set over the existing `infer serve` transport, instead of weights -- the coordinator-based FedAvg/Multi-Krum/DiLoCo path in `coordinator.py` requires matching LoRA shape against the same base model and can't include heterogeneous peers at all. Consensus-scores each peer's answer against the others' (text-domain analogue of `aggregation.py`'s Multi-Krum), updates a local `PeerTrustStore` keyed per `(peer_id, domain)` -- not one scalar per peer -- so future rounds prefer proven teachers *for that kind of question*, and emits an ordinary `continual_guard.py`-compatible distillation corpus -- so a bad/malicious peer can only nudge training data, never bypass the regression/promotion gate. `classify_domain()` buckets a live question the same way `domain_of_probe()` buckets a training probe (by the probe id's `<domain>-NN` prefix), so both paths draw on the same reputation. `select_probes_for_round()` samples a deterministic, date-seeded subset of the shared probe set per round instead of always exposing the full fixed list. Reachable from the web UI's Network tab ("Peer teaching") as well as the CLI. The same transport/consensus/trust machinery also powers a *live* path (`ask_swarm()`): an "Ask the swarm too" button on every chat answer queries domain-trust-ranked peers concurrently with the user's actual question and shows their answers alongside Ickle's own, so collective intelligence isn't limited to future training rounds. `infer ask` (`inference_swarm.py`) also reads this same trust store to rank offers instead of picking the first one found. See `docs/CODISTILLATION.md`.
...
1. Strengthen decentralized networking beyond current DHT/signed-announcement swarm path (peer quality, resiliency). NAT traversal itself (STUN public-IP discovery + UPnP auto port-forwarding) is implemented -- `src/federated/nat_traversal.py`; still open: a documented way for communities to run/share their own bootstrap peers at scale (see `docs/BOOTSTRAP_NODE.md`) since there is deliberately no Ickle-operated public directory.
...
6. ~~Extend co-distillation with per-domain trust and probe-set rotation~~ -- done: `PeerTrustStore` (`src/federated/codistill.py`) now tracks an EMA per `(peer_id, domain)` pair instead of one scalar per peer -- a peer's code-teaching trust and creative-writing trust are tracked and used separately, in both the training-round path and the live `ask_swarm()` path, and old flat-scalar trust files migrate transparently on load. `select_probes_for_round()` picks a deterministic, date-seeded random subset of the shared probe set each round (`--sample-size`, wired to 14/24 by default in the web UI's queued round) so the full probe list is never all exposed every time.
