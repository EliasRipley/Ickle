...
- `src/disagreement_curriculum.py` -- accumulates polarity-conflicting claim clusters from `epistemics.build_collective_view()` (surfaced by both the live "ask the swarm too" chat path and periodic co-distillation rounds, previously discarded after each call) into a durable, deduplicated, peer-count-ranked queue (`data/commons/disagreements.json`). Unresolved entries feed a hedge-training corpus (`auto_include_disagreement_hedges`, on by default) so `continual_guard_step` teaches the model to say peers disagree rather than confidently pick a side; the moment an entry's exact claim text is corrected/adopted in the Epistemic Commons, it drops out of this queue and into `verified_corrections.py`'s confident-correction corpus instead -- a claim is never trained to be both hedged and asserted. `GET /api/disagreements/status`; Control room -> Network -> Epistemic Commons -> "What the swarm disagrees about".
